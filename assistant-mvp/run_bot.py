...
def run_bot():
    ...
    if not token or token == "your_bot_token":
        logger.error(f"Missing or invalid TELEGRAM_BOT_TOKEN: '{token}'. Please set it in .env")
        return
    ...
    job_queue.run_repeating(check_reviews_job, interval=60*20, first=10) # 300s = 5 phút
